...
fn return_map() {
    ...
    expected
        .map_insert(String::from("foo"), "bar".into())
        .unwrap();
...
fn nested_map() {
    ...
    let mut expected = Value::make_map();
    expected
        .map_insert(String::from("foo"), Value::make_map())
        .unwrap();
    assert_eq!(result, expected.into());
}
...
fn type_check_str_map() {
    let meta_map = TypeDefinition::Map(
...
fn type_check_u64_map() {
    let meta_map = TypeDefinition::Map(
        Box::new(TypeDefinition::Primitive(PrimitiveType::String)),
        Box::new(TypeDefinition::Primitive(PrimitiveType::U64)),
    );
